...
# ===========================================================================
# 0x42 ADD A,$nn   (2 cyc)
.opcode page0 ADD A,$nn
routine ADD A,$nn:
  SCR1 <- [PC]; PC++
  A <- A + SCR1 : nzvch ; return to fetch

# 0x43 ADD A,(X)   (3 cyc)
.opcode page0 ADD A,(X)
routine ADD A,(X):
  MAR  <- X
  SCR1 <- [MAR]
  A <- A + SCR1 : nzvch ; return to fetch
...
# 0x4a ADD B,$nn   (2 cyc)
.opcode page0 ADD B,$nn
routine ADD B,$nn:
  SCR1 <- [PC]; PC++
  B <- B + SCR1 : nzvch ; return to fetch

# 0x4b ADD B,(X)   (3 cyc)
.opcode page0 ADD B,(X)
routine ADD B,(X):
  MAR  <- X
  SCR1 <- [MAR]
  B <- B + SCR1 : nzvch ; return to fetch
...
# 0x52 SUB A,$nn   (2 cyc)
.opcode page0 SUB A,$nn
routine SUB A,$nn:
  SCR1 <- [PC]; PC++
  A <- A - SCR1 : nzvc ; return to fetch

# 0x53 SUB A,(X)   (3 cyc)
.opcode page0 SUB A,(X)
routine SUB A,(X):
  MAR  <- X
  SCR1 <- [MAR]
  A <- A - SCR1 : nzvc ; return to fetch
...
# 0x5a SUB B,$nn   (2 cyc)
.opcode page0 SUB B,$nn
routine SUB B,$nn:
  SCR1 <- [PC]; PC++
  B <- B - SCR1 : nzvc ; return to fetch

# 0x5b SUB B,(X)   (3 cyc)
.opcode page0 SUB B,(X)
routine SUB B,(X):
  MAR  <- X
  SCR1 <- [MAR]
  B <- B - SCR1 : nzvc ; return to fetch
...
# 0x62 CMP A,$nn   (2 cyc)
.opcode page0 CMP A,$nn
routine CMP A,$nn:
  SCR1 <- [PC]; PC++
  _ <- A - SCR1 : nzvc ; return to fetch

# 0x63 CMP A,(X)   (3 cyc)
.opcode page0 CMP A,(X)
routine CMP A,(X):
  MAR  <- X
  SCR1 <- [MAR]
  _ <- A - SCR1 : nzvc ; return to fetch
...
# 0x68 CMP B,$nn   (2 cyc)
.opcode page0 CMP B,$nn
routine CMP B,$nn:
  SCR1 <- [PC]; PC++
  _ <- B - SCR1 : nzvc ; return to fetch

# 0x69 CMP B,(X)   (3 cyc)
.opcode page0 CMP B,(X)
routine CMP B,(X):
  MAR  <- X
  SCR1 <- [MAR]
  _ <- B - SCR1 : nzvc ; return to fetch
...
# 0x6d AND A,$nn   (2 cyc)
.opcode page0 AND A,$nn
routine AND A,$nn:
  SCR1 <- [PC]; PC++
  A <- A & SCR1 : nz, v=0 ; return to fetch

# 0x6e AND A,(X)   (3 cyc)
.opcode page0 AND A,(X)
routine AND A,(X):
  MAR  <- X
  SCR1 <- [MAR]
  A <- A & SCR1 : nz, v=0 ; return to fetch
...
# 0x73 AND B,$nn   (2 cyc)
.opcode page0 AND B,$nn
routine AND B,$nn:
  SCR1 <- [PC]; PC++
  B <- B & SCR1 : nz, v=0 ; return to fetch

# 0x74 AND B,(X)   (3 cyc)
.opcode page0 AND B,(X)
routine AND B,(X):
  MAR  <- X
  SCR1 <- [MAR]
  B <- B & SCR1 : nz, v=0 ; return to fetch
...
# 0xb3 BRN rel8   (2 cyc)
.opcode page0 BRN rel8
routine BRN rel8:
  MDR  <- [PC]; PC++                # rel8 displacement
  return to fetch

# 0xb4 BHI rel8   (3 cyc)
.opcode page0 BHI rel8
routine BHI rel8:
  MDR  <- [PC]; PC++                # rel8 displacement
...
.opcode page0 LEA X,X+A
routine LEA X,X+A:
  SCR1 <- sext(A)
  X <- X + SCR1 : z ; return to fetch

# 0xde LEA X,X+B   (2 cyc)
.opcode page0 LEA X,X+B
routine LEA X,X+B:
  SCR1 <- sext(B)
  X <- X + SCR1 : z ; return to fetch

# 0xdf LEA X,X+D   (2 cyc)
.opcode page0 LEA X,X+D
routine LEA X,X+D:
  SCR1 <- D
  X <- X + SCR1 : z ; return to fetch

# 0xe0 LEA X,X+   (1 cyc)
.opcode page0 LEA X,X+
routine LEA X,X+:
...
.opcode page1 LBRN rel16
routine LBRN rel16:
  SCR1.low  <- [PC]; PC++            # rel16 displacement low
  return to fetch

# 0x21 LBHI rel16   (3 cyc)
.opcode page1 LBHI rel16
routine LBHI rel16:
  SCR1.low  <- [PC]; PC++            # rel16 displacement low
...
# ===========================================================================
# 0x46 ST A,(SP)   (2 cyc)
.opcode page1 ST A,(SP)
routine ST A,(SP):
  MAR  <- SP
  [MAR] <- A : nz, v=0 ; return to fetch

# 0x47 ST B,(SP)   (2 cyc)
.opcode page1 ST B,(SP)
routine ST B,(SP):
  MAR  <- SP
  [MAR] <- B : nz, v=0 ; return to fetch
...
# 0xb2 EOR A,$nn   (2 cyc)
.opcode page1 EOR A,$nn
routine EOR A,$nn:
  SCR1 <- [PC]; PC++
  A <- A ^ SCR1 : nz, v=0 ; return to fetch

# 0xb3 EOR B,$nn   (2 cyc)
.opcode page1 EOR B,$nn
routine EOR B,$nn:
  SCR1 <- [PC]; PC++
  B <- B ^ SCR1 : nz, v=0 ; return to fetch

# 0xb4 EOR A,($nnnn)   (5 cyc)
.opcode page1 EOR A,($nnnn)
routine EOR A,($nnnn):
  SCR1.low  <- [PC]; PC++
  SCR1.high <- [PC]; PC++
  MAR  <- SCR1
