#!/usr/bin/env bash
# Build + run the register-file integration testbench under Icarus (-gspecify, TIMED; D-47).
# Directed microprogram exercises D/X/Y/USP/SSP + ACTIVE_SP banking through the whole datapath.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
python3 "$ROOT/sim/tb/regfile/mk_regfile_image.py"
IMG="$ROOT/microcode/build/regfile_test.hex"
OUT=/tmp/blip_regfile
mkdir -p "$OUT"
iverilog -g2012 -gspecify -Wall -D IMG="\"$IMG\"" -o "$OUT/tb" \
    ...
    "$ROOT/hdl/cpu.v" \
    "$ROOT/sim/tb/regfile/tb_regfile.v"
vvp "$OUT/tb"
