#!/usr/bin/env bash
# Build + run the read-posts-on-Z testbench under Icarus (-gspecify, TIMED; D-47).
# Proves a memory read posts its byte on Z and latches a register + N/Z in one microword.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
python3 "$ROOT/sim/tb/ldz/mk_ldz_image.py"
IMG="$ROOT/microcode/build/ldz_test.hex"
OUT=/tmp/blip_ldz
mkdir -p "$OUT"
