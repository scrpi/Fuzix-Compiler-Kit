...
//
//   two ttl_283 (carry-cascaded) form an 8-bit adder; one ttl_574 holds acc.
//
...
module add_accum (
    ...
    ttl_283 add_lo (.S(sum[3:0]), .C4(c_lo), .A(acc[3:0]), .B(din[3:0]), .C0(1'b0));
    ttl_283 add_hi (.S(sum[7:4]), .C4(c_hi), .A(acc[7:4]), .B(din[7:4]), .C0(c_lo));
    ttl_574 reg8   (.Q(acc), .D(sum), .CLK(clk), .OE_n(oe_n));
endmodule
