;
;	__remeql — long (32-bit) signed `%=` op-assign helper.
;
...
;	(signed; C99 remainder takes the dividend's sign), writes the remainder
;	back, pops its own pointer + return, and returns via JMP X with the
;	remainder in D:Y.
;
;	Delegates to the signed value helper __reml.  The divisor is preserved in
;	its own stack slot and reloaded into D:Y just before the call, since
...
__remeql:
	...
	LD A,(X+3)
	ST A,(SP+3)
	LD D,(SP+4)		; reload divisor into D:Y for __reml
	LD Y,(SP+6)
	JSR __reml		; pops LHS(4)+ret(2); remainder -> D:Y.  Back:
