;
;	__xoreql — 32-bit (long) `^=` op-assign helper.
;
...
;
;	XOR has no A,(SP+n) form, so each RHS byte is loaded into A and combined
;	against *p through the A,(X+n) form (xor is commutative).
;
	.export __xoreql
	.code
__xoreql:
