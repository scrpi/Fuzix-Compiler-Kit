...
- **R-HW-2** (⟸ G1, G6) — All core logic shall be drawn from a single logic
...
- **R-CPU-7** (⟸ G3, G7) — On reset the CPU shall enter a defined, deterministic
...
- **R-MEM-7** (⟸ G3, G7) — On reset, address translation shall default to a
...
- **R-CTRL-1** (⟸ G5) — Instruction decoding and sequencing shall be defined by
  stored microcode rather than fixed random logic, so the instruction set can be
  developed, corrected, and extended without rewiring.
- **R-CTRL-2** (⟸ G5, G6) — The runtime microcode store shall be fast enough not
  to dominate the cycle time at the target clock (R-CLK-1).
- **R-CTRL-3** (⟸ G5) — Microcode shall be modifiable at the bench during
  development, yet retained across power cycles so the machine boots unattended.

## Timing (CLK)

- **R-CLK-1** (⟸ G6) — The design shall target a continuous clock of about
...
- **R-CLK-2** (⟸ G6) — The microarchitecture shall prefer registered/pipelined
...
- **R-DBG-1** (⟸ G7) — Every bus and architectural register shall be
  continuously displayable in a stable, legible form, whether the machine is
  free-running, single-stepped, or stopped.
- **R-DBG-2** (⟸ G7) — A front panel shall be able to halt the CPU, take control
  of the buses, and examine and deposit individual memory locations by hand.
- **R-DBG-3** (⟸ G7) — The machine shall be bootstrappable from the front panel
  alone: code can be entered and execution started with no external tools.
- **R-DBG-4** (⟸ G7) — The CPU shall support run/stop control and single-stepping
  (at least per instruction; preferably also per microstep).
- **R-DBG-5** (⟸ G7, G8) — The front panel shall be the sole exception to the
...
- **R-IF-1** (⟸ G8) — All interaction between the CPU and the *functional* parts
...
- **R-IF-2** (⟸ G8) — The interface shall define an address bus, a data bus, and
...
- **R-IF-3** (⟸ G8, G3) — The interface shall carry the asynchronous system
...
- **R-IF-4** (⟸ G8, G7) — The interface shall provide a bus-request / bus-grant
...
- **R-IF-6** (⟸ G8) — The interface shall be stable: changes to the CPU's
...
| **G5** Microcoded control | R-CTRL-1, R-CTRL-2, R-CTRL-3 |
| **G6** ~10 MHz | R-HW-2, R-CLK-1, R-CLK-2, R-CTRL-2 |
| **G7** Functional blinkenlights | R-DBG-1…5, R-IF-4, R-CPU-7, R-MEM-7 |
| **G8** Defined CPU/system interface | R-IF-1…4, R-IF-6, R-DBG-5 |
