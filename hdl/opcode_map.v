// opcode_map — the opcode→start-address map: the 2 byte-wide SRAMs that translate
...
//
// The map holds 512 entries ({PAGE, IR} = 9 bits); each entry's 12-bit µPC start address
...
//   2x is61c64      -> the map SRAMs (8 K part, A12 grounded; D-49). /CE low; /OE = oe_n
//                      (= loading); /WE the per-chip strobe.
`timescale 1ns/1ps
`default_nettype none
module opcode_map (
    input  wire [11:0] loader_addr,  // boot: the shared control-store address (loader counter)
    input  wire        dispatch_page,// run: opcode-map page (control-word DISPATCH_PAGE)
    ...
    output wire [7:0]  map_lo,       // entry low byte  (SRAM 0)
    output wire [7:0]  map_hi        // entry high byte (SRAM 1; only the low 4 bits are used)
);
    // address mux: run = {PAGE, IR} (512-entry map), boot = loader address
    ...
    // the 2 map SRAMs + their boot-write buffers
    wire [7:0] io [0:1];
    genvar g;
