/*
 *	Large model 8080 differs slightly
 */

#include "compiler.h"

unsigned target_ptr = ULONG;
...
{
	n->type = PTRTO + type;
	/* We could do with a way to say "do 16bit maths on a 32bit value" somewhere
	   or pick it up in the backend would be better */
	n = tree(T_PLUS, n, make_constant(off, ULONG));
	n->type = type;
	return n;
}
...
unsigned target_register(unsigned type, unsigned storage)
{
	if (!bc_free || type >= CLONG)
		return 0;
        /* Unlike the base 8080 we can't use BC as a pointer because it doesn't fit */
	if (PTR(type) == 0) {
		bc_free = 0;
		/* Tell the backend */
		if (storage == S_AUTO)
